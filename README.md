...
- pending nonce calculation, txpool inspection, 10 percent same-sender/same-nonce pending transaction replacement, pending transaction lookup, pending transaction filter polling, and WebSocket pending transaction hash subscriptions for uncommitted transactions
...
- `GET /tx/{hash}`
- `GET /txpool`
- `POST /tx` for signed transactions, including `transfer`, `batch`, `deploy`, `call`, `wasm.upload`, staking, validator, and governance transaction types. If a pending transaction already has the same sender and nonce, ChainLab accepts a replacement only when the new legacy gas price or both EIP-1559 fee caps are bumped by at least 10 percent.
- `POST /tx/raw`
- `POST /faucet`
- `POST /chain/produce`
- `POST /peer/tx`

...
- `GET /rpc/ws` supports WebSocket JSON-RPC `eth_subscribe` for `newHeads` and `eth_unsubscribe`. New block headers are pushed after local block production and canonical peer block import. This is a node-local development subscription path; log and pending-transaction WebSocket subscriptions remain future work.

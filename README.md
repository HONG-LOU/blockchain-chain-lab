...
- local block explorer pages for head, finality, recent blocks, committed/pending/queued transactions, accounts, validators, bounded mempool status, and indexed recent contract events
...
- a pinned CometBFT v0.39.3 ABCI++ application with strict genesis/consensus-parameter binding, deterministic proposal replay, candidate-only finalize, duplicate-vote, same-height light-client equivocation, and cross-height forward-lunatic evidence-driven `chainlab-v2` validator removal, Pebble-backed incremental history profiles, and verified snapshot restore

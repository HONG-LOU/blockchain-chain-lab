...
- `GET /rpc/ws` supports WebSocket JSON-RPC `eth_subscribe` for `newHeads` and filtered `logs`, plus `eth_unsubscribe`. New block headers and matching EVM-style logs are pushed after local block production and canonical peer block import. This is a node-local development subscription path; pending-transaction WebSocket subscriptions remain future work.

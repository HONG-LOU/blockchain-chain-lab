...
| Networking | The production path has independent CometBFT homes, full-mesh persistent peers, flood-mempool gossip, real four-validator rounds, 3-of-4 progress, targeted unavailable, connected-but-delayed, and malformed-proposal round-0 proposer recovery at later rounds, 2-of-4 quorum-loss recovery, symmetric 2+2 P2P partition isolation/healing, restart, block sync, destructive-data state sync via two light-client RPC sources, duplicate-vote evidence transitions, Comet-verified same-height light-client equivocation, and cross-height forward-lunatic verification followed by three-validator removal. | Initial CometBFT private-network block/state-sync/evidence path; validator outage, proposal-construction delay, invalid proposal rejection, simultaneous removal, light-client equivocation, forward-lunatic evidence, and symmetric topology partition behavior are tested, while discovery policy, dynamic packet faults/asymmetric partitions, sentries, and load remain incomplete | Complete discovery/peer policy, amnesia evidence and external light-client detection/sourcing, packet-level delay/drop/reorder coverage, asymmetric partitions, non-removal power/admission changes, sentry topology, additional Byzantine tests, ingress bounds, and load validation. |
...
| Validator lifecycle | The local harness and `chainlab-v1` remain genesis-fixed with join/leave rejected. `chainlab-v2` persistently binds consensus address/account/pubkey/power, exact evidence retention/slash ratios, epoch-bound updates, deterministic ordering, and Comet `H+2`. Genesis certificates pre-authorize stake-derived admissions; optional `runtime_admissions` accepts candidate-paid joins authorized by active voting power above two thirds and bound to the preceding validator root/height. Optional delayed unbonding follows evidence removal. All additions default off/omitted for legacy root compatibility. Direct tests cover runtime join through candidate proposal; real four-process evidence tests prove removal transitions. | Initial evidence removal/unbonding plus offline and runtime certified stake-derived admission; runtime admission still lacks real multi-process evidence, while re-entry, power changes, voluntary leave/rewards/economics, and signing remain incomplete | P0: add real runtime-admission transition evidence, re-entry/power changes, voluntary-leave unbonding/rewards, governance authority, and remote-signer monotonic last-sign state. |

...
| WASM | Static function-body/host-resource charge plus wall-clock cancellation | Consensus-unsafe for potentially unbounded guests | P0: deterministic runtime instruction/epoch fuel and memory/table/stack/host-I/O/storage-growth limits. Wall-clock timeout may kill local work but must never decide receipt/state. Reject modules that cannot be deterministically bounded. |
| Persistence | Whole-chain JSON snapshot written via temporary rename | Development snapshot, no database guarantees | P0: embedded KV with atomic batches, schema/version, WAL/reopen, corruption and fault injection, state snapshots, and explicit pruned/full/archive modes. |
...
| Txpool | Pending/queued, nonce promotion, 10% replacement | Functional local txpool | P0: capacity, per-account quota, deterministic eviction, journal/restart, local policy, DoS/load tests, and authorization-principal-safe replacement. |
| RPC/indexing | REST, EVM-shaped JSON-RPC, filters, WebSocket, local event index | Useful development interface | P0: body/batch/log-range limits, rate limits, timeouts, WS backpressure, pagination, external indexer boundary, canonical reorg semantics. |
...
- Preserve the local PoA implementation only as a fast deterministic test harness; differential-test it against the ABCI++ application state transition.

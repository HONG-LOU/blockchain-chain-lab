...
| ABCI++ application lifecycle | CometBFT v0.39.3 is pinned with signed checksum evidence. The in-memory `chainlab-v1` app strictly binds genesis/consensus parameters, implements the full interface with bounded proposal/mempool work, publishes finalized candidates only on commit, commits execution metadata into app hash, and passes fresh-process plus local-harness receipt/root differential tests. Snapshot restore is explicitly rejected rather than reported as implemented. | Initial authoritative application state-transition lifecycle; no real CometBFT network or durable restart yet | P0: run through real socket/builtin CometBFT nodes, persist/restore height and app hash atomically, and prove validator updates, evidence slashing, snapshots/state sync, block sync, P2P rounds, partitions, and rolling restart in multi-process networks. |
...
| Execution failure and native gas | Included deterministic failures produce stable failed receipts, roll back business state, consume sender nonce, settle actual gas, charge paymasters for sponsored execution, and consume the full gas limit on OOG. Native contracts use sealed `chainlab-native-v1` metering with fixed input/storage/event/output bounds and exact fresh-process vectors. The initial ABCI++ path differentially matches local-harness successful/failed receipts and roots. | Initial deterministic settlement and native-metering implementation; tested through an in-memory ABCI++ lifecycle but not activated on a production network | P0: freeze these rules in an activated protocol version, differential-test real multi-process ABCI++ block/restart/sync replay, broaden fee/nonce/rollback property and fuzz evidence, load-test resource ceilings, and complete external review. |
...
| Security evidence | The final ABCI++ application tree passes the full unit/race suites, focused ABCI race, vet, tidy-diff, production build, fresh-process vectors, demo, diff checks, and cached `govulncheck` v1.6.0 with zero reachable vulnerabilities. CometBFT's two module sums match two signed sumdb endpoints and 60/60 new transitive sums match its upstream `go.sum`; Windows directory verification is not green because its filesystem normalizes an upstream trailing-space workflow filename. Fault/load/soak, clean Linux module verification, cross-target, and independent-audit gates have not been completed. | Strong local regression and signed dependency evidence; insufficient for hostile deployment | P0: broader fuzz/property/fault/cross-target evidence, malicious WASM/RPC/raw-tx corpora, benchmarks/load/soak, maintained dependency scanning, clean Linux verification, threat model, and external review. |

...
| Networking | The production path has independent CometBFT homes, full-mesh persistent peers, flood-mempool gossip, real four-validator rounds, 3-of-4 progress, targeted unavailable, connected-but-delayed, and malformed-proposal round-0 proposer recovery at later rounds, 2-of-4 quorum-loss recovery, symmetric 2+2 P2P partition isolation/healing, restart, block sync, destructive-data state sync via two light-client RPC sources, runtime validator admission, duplicate-vote evidence transitions, Comet-verified same-height light-client equivocation, and cross-height forward-lunatic verification followed by three-validator removal. | Initial CometBFT private-network block/state-sync/admission/evidence path; validator outage, proposal-construction delay, invalid proposal rejection, runtime admission, simultaneous removal, light-client equivocation, forward-lunatic evidence, and symmetric topology partition behavior are tested, while discovery policy, dynamic packet faults/asymmetric partitions, sentries, and load remain incomplete | Complete discovery/peer policy, amnesia evidence and external light-client detection/sourcing, packet-level delay/drop/reorder coverage, asymmetric partitions, re-entry/non-removal power changes, sentry topology, additional Byzantine tests, ingress bounds, and load validation. |
...
| RPC/indexing | REST, EVM-shaped JSON-RPC, filters, WebSocket, local event index, and a pool-aware explorer. Ordinary HTTP work, POST, and JSON-RPC each have 32-slot bounds; POST is capped at 16 MiB and JSON-RPC at 5 MiB/100 batch items/16 MiB batch responses. Txpool sources stop at 8 MiB; fee history at 1,024 blocks. Logs stop at 10,000 blocks, 1,000 results, 8 MiB, 256 canonical addresses, four topic positions, 256 canonical alternatives, and 64 KiB installed definitions. Filters use random IDs, total 1,024 with active five-minute expiry; pending filters stop at 64 and 65,536 retained hashes. WebSockets stop at 64 connections, 64 subscriptions per connection, 1,024 total, 64 log subscriptions, and 100,000 log matches per block, with 30-minute subscription expiry, heartbeat/read/write deadlines, and bounded queues. Pending/queued explorer links resolve without fabricated receipts. `eth_call` accepts only `latest` and reads an immutable state view without holding the node lock. | Resource-bounded development interface; load and hostile-client evidence remain incomplete | P0: principal-aware rate limits and measured disconnect/backpressure policy, dynamic state-executing gas estimation for WASM deploy/call/batch-call, pagination, external indexer boundary, canonical reorg semantics, and load/soak evidence. |
...
| Validator lifecycle | The local harness and `chainlab-v1` remain genesis-fixed with join/leave rejected. `chainlab-v2` persistently binds consensus address/account/pubkey/power, exact evidence retention/slash ratios, epoch-bound updates, deterministic ordering, and Comet `H+2`. Genesis certificates pre-authorize stake-derived admissions; optional `runtime_admissions` accepts metered candidate-paid joins authorized by active voting power above two thirds and bound to the preceding validator root/height. Optional delayed unbonding follows evidence removal. All additions default off/omitted for legacy root compatibility. Direct tests cover same-block admissions and candidate proposal; a real four-process test proves the quorum-halt, join, positive-power update, expanded set, convergence, and continued-progress transition. | Initial evidence removal/unbonding plus offline and runtime certified stake-derived admission; re-entry, power changes, voluntary leave/rewards/economics, and signing remain incomplete | P0: add re-entry/power changes, voluntary-leave unbonding/rewards, governance authority, and remote-signer monotonic last-sign state. |
...
5. **Resource and abuse controls**
   ...
   - Load-test active filter/subscription expiry, pending-hash budgets, WebSocket heartbeat/write timeout, bounded queues and matching budget; add principal-aware rate limits, peer/ABCI/P2P ingress bounds, indexer-queue bounds, and limits for all future internal work queues.
...
- Preserve the implemented ChainLab ABCI++/CometBFT authoritative block, block-sync, state-sync, snapshot, single/simultaneous duplicate-vote slashing and epoch removal, targeted missing/delayed/invalid-proposer recovery, quorum-loss, and symmetric partition/heal lifecycle while adding certified admission/re-entry, dynamic message faults/asymmetric partitions, additional Byzantine evidence, and upgrade compatibility in multi-process networks.

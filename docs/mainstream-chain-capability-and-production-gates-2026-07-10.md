...
| Security evidence | The final integrated hardening tree passes the full unit suite, race detector, vet, module verification, production build, fresh-process vectors, demo, diff checks, and the cached `govulncheck` v1.6.0 reachable-symbol scan. Fault/load/soak, cross-target, and independent-audit gates have not been completed. | Strong local regression evidence; insufficient for hostile deployment | P0: broader fuzz/property/fault/cross-target evidence, malicious WASM/RPC/raw-tx corpora, benchmarks/load/soak, maintained dependency scanning, threat model, and external review. |

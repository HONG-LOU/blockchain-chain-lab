...
|---|---|---|---|
| State transition and roots | Executor, canonical hashing, replay tests, account-storage caps, and V3 domain-separated transaction/receipt/full-flat-state Merkle roots with current/historical inclusion proofs and fixed fresh-process vectors | Initial versioned root/proof protocol with incomplete scale and cross-target evidence | Add broader root/state-transition properties, sparse/non-inclusion proofs, cross-architecture vectors, a second independent verifier, trusted-header integration, and load/external review. |
| ABCI++ application lifecycle | CometBFT v0.39.3 is pinned with signed checksum evidence. `chainlab-v1` binds the original lifecycle; V2 adds validator identity/evidence slashing and epoch removal. A genesis-committed V3 schedule returns the Comet app-version update at `A-1` and activates proof roots at `A`. Real four-validator tests prove duplicate-vote removal and V3 version/root/proof convergence. | Initial authoritative crash-consistent multi-process application/consensus/block-sync/state-sync/evidence-removal/scheduled-upgrade lifecycle | P0: runtime-authorized upgrade governance and multi-upgrade/rollback policy; certified validator admission/re-entry and economics; partitions, additional Byzantine faults, load, rolling operations, and validator signing. |
...
| Persistence | Application Store V2 uses Pebble with one synchronized batch per height. It atomically binds a flat live state, mutation-journal-derived sets/deletes, periodic checkpoints, commitment/app hash, raw transactions, receipts, block index, current pointer, and minimum history. Store clones preserve semantic dirty keys until durable commit; ordinary heights encode only touched entries, while checkpoints compare that delta against a complete flatten/diff and fail closed on mismatch. Startup verifies genesis/profile identity, manifest checksums/counts, roots, the current live state, and the retained checkpoint boundary. Deterministic V1 migration, archive/full/pruned profiles, historical reads, compaction, consistent checkpoint backup/open, state-sync rebasing, abrupt-exit/forced-kill recovery, and corruption rejection are tested. Format-1 snapshots remain independently bound to genesis and trusted app hash. | Crash-consistent mutation-aware disk history with explicit retention and migration guarantees; flat integrity roots, V3 full-state proof roots, and checkpoints still traverse current state, and evidence is Windows-focused | P0: incremental authenticated flat/proof-root accumulation, production-size compaction/load/soak evidence, broader filesystem power-loss coverage, operational restore drills, and external rollback protection. |
| Verifiability | V3 commitments use exact-total/index/domain-separated Merkle roots for canonical transactions, receipts, and all flat-state entries. Current and retained-height transaction/receipt/account proofs are verified by a standalone trusted-root CLI; fixed vectors, restart, snapshot/state sync, tamper rejection, and a real four-node upgrade/proof flow pass. | Initial inclusion proofs and minimal verifier; account is the only public state-proof route and trust-anchor integration is external | P0: sparse/non-inclusion proofs, broader state routes, bounded proof serving, trusted light-client/header integration, second independent implementation, bridge/wallet/explorer consumption, and external review. |
| Networking | The production path has independent CometBFT homes, full-mesh persistent peers, flood-mempool gossip, real four-validator rounds, 3-of-4 progress, restart, block sync, destructive-data state sync via two light-client RPC sources, and duplicate-vote evidence broadcast/propagation followed by a live four-to-three validator transition. | Initial CometBFT private-network block/state-sync/evidence path; discovery policy, partitions, sentries, and load remain incomplete | Complete discovery/peer policy, light-client-attack and partition coverage, sentry topology, additional Byzantine tests, ingress bounds, and load validation. |
| Txpool | Pending/queued with 2 MiB transaction, 128 MiB/4,096 total, 2,048 queued, 64-per-sender and 64-nonce-gap caps; principal-safe replacement, block-gas admission, bounded batches, canonical-import revalidation, and production/import sequencing of included failures before the next nonce | Bounded functional local txpool | P0: deterministic fee-aware eviction/TTL, journal/restart, incremental accounting, gas-bounded proposal simulation/revalidation, orphaned-transaction reinsertion after reorg, local policy, and sustained DoS/load evidence. |
...
4. **Protocol lifecycle**
   ...
   - Preserve the implemented genesis-committed V2-to-V3 height activation, Comet app-version transition, deterministic root migration, and incompatible-node rejection; add runtime authority, multiple upgrades, binary manifests, and rollback limits.
...
8. **Verifiability boundary**
   - Preserve V3 transaction/receipt/account inclusion proofs and the standalone trusted-root verifier; add non-inclusion/broader state proofs, light-client trust anchoring, a second independent implementation, and production proof-serving evidence.

## Parallel Implementation Tracks

- Preserve the implemented ChainLab ABCI++/CometBFT authoritative block, block-sync, state-sync, snapshot, duplicate-vote slashing, and epoch-removal lifecycle while adding certified admission/re-entry, partitions, additional Byzantine evidence, and upgrade compatibility in multi-process networks.
- Preserve the implemented mutation-aware incremental store, checkpoint cross-checks, migrations, retention profiles, historical reads, compaction, and backup/open behavior; add incremental authenticated root maintenance and complete Linux capacity, restore, fault, and rollback evidence before public testnet.

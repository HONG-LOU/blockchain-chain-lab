...
| Networking | The production path has independent CometBFT homes, full-mesh persistent peers, flood-mempool gossip, real four-validator rounds, 3-of-4 progress, targeted missing round-0 proposer recovery at a later round, 2-of-4 quorum-loss recovery, symmetric 2+2 P2P partition isolation/healing, restart, block sync, destructive-data state sync via two light-client RPC sources, duplicate-vote evidence transitions, and Comet-verified same-height light-client equivocation followed by three-validator removal. | Initial CometBFT private-network block/state-sync/evidence path; validator outage, missing-proposer round advancement, simultaneous removal, light-client equivocation, and symmetric topology partition behavior are tested, while discovery policy, dynamic message faults/asymmetric partitions, sentries, and load remain incomplete | Complete discovery/peer policy, forward/amnesia light-client cases, packet-level delay/drop/reorder coverage, asymmetric partitions, delayed-but-connected proposers, non-removal power/admission changes, sentry topology, additional Byzantine tests, ingress bounds, and load validation. |
...
- Preserve the implemented ChainLab ABCI++/CometBFT authoritative block, block-sync, state-sync, snapshot, single/simultaneous duplicate-vote slashing and epoch removal, targeted missing-proposer, quorum-loss, and symmetric partition/heal lifecycle while adding certified admission/re-entry, dynamic message faults/asymmetric partitions, additional Byzantine evidence, and upgrade compatibility in multi-process networks.

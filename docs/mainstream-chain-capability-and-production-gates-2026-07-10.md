...
| Persistence | Application Store V2 uses Pebble with one synchronized batch per height. It atomically binds a flat live state, incremental sets/deletes, periodic checkpoints, commitment/app hash, raw transactions, receipts, block index, current pointer, and minimum history. Startup verifies genesis/profile identity, manifest checksums/counts, roots, the current live state, and the retained checkpoint boundary. Deterministic V1 migration, archive/full/pruned profiles, historical reads, compaction, consistent checkpoint backup/open, state-sync rebasing, abrupt-exit/forced-kill recovery, and corruption rejection are tested. Format-1 snapshots remain independently bound to genesis and trusted app hash. | Crash-consistent incremental disk history with explicit retention and migration guarantees; commit-time flat snapshot/diff remains O(current state) and evidence is Windows-focused | P0: mutation-aware incremental delta generation, production-size compaction/load/soak evidence, broader filesystem power-loss coverage, operational restore drills, and external rollback protection. |
...
- Preserve the implemented incremental store, migrations, retention profiles, historical reads, compaction, and backup/open behavior; remove commit-time full-state diff cost and complete Linux capacity, restore, fault, and rollback evidence before public testnet.

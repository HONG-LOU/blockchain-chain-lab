...
| Validator lifecycle | The local harness and `chainlab-v1` remain genesis-fixed with join/leave rejected. `chainlab-v2` persistently binds consensus address/account/pubkey/power, exact evidence retention and slash ratios, one validator-height tombstone, overflow-safe stake penalty, epoch-bound updates, deterministic multi-update ordering, and the Comet `H+2` delay. Ordered chain-bound genesis certificates with `2/3+1` genesis-validator signatures can pre-authorize future admissions with `floor(genesis_stake/1000)` power. Explicit `unbonding_epochs>=2` permits withdrawal only after that many epochs following evidence removal; zero/legacy state stays permanently locked. Certified candidates survive restart and are accepted as proposers only during their active interval. Direct restart/snapshot tests and real four-process evidence propagation prove single 4-to-3 and simultaneous 4-to-2 removal with post-transition liveness. | Initial evidence-driven removal/unbonding plus offline genesis-certified, stake-derived admission; runtime admission/re-entry and power changes, voluntary leave/rewards/economics, and signing operations remain incomplete | P0: implement runtime certified admission/re-entry and power changes, voluntary-leave unbonding/rewards, governance authority, and remote-signer monotonic last-sign state; do not re-enable ad hoc state-driven membership. |

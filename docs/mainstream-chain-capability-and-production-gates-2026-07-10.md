...
|---|---|---|---|
| State transition and roots | Executor, canonical hashing, replay tests, and account-storage caps of 4,096 entries, 256 bytes per key, 64 KiB per value, and 8 MiB total per account with clone/restore-safe O(1) derived byte accounting | Initial production protocol implementation with incomplete evidence | Add property tests, cross-process/cross-architecture vectors, inclusion proofs, and an independent verifier. |
| ABCI++ application lifecycle | CometBFT v0.39.3 is pinned with signed checksum evidence. `chainlab-v1` strictly binds genesis/consensus parameters, bounds proposal/mempool work, and publishes only after a synchronized Pebble WAL batch atomically commits height/app hash/state/transactions/receipts/index. Official socket and four-validator process tests prove P2P rounds, transaction commitment, 3-of-4 progress, Comet restart, block sync, durable app restart, fresh-app replay, destructive-data state sync through two light-client RPC sources, common-height block/app-hash equality, and state convergence. | Initial authoritative crash-consistent multi-process application/consensus/block-sync/state-sync lifecycle | P0: prove validator updates, evidence slashing, partitions, Byzantine faults, load, and rolling upgrades; complete storage retention/migration and validator-signing gates. |
...
| Persistence | The ABCI path uses Pebble with one synchronized WAL batch per committed height. Versioned prefixes and a current pointer atomically bind canonical per-key state, commitment/app hash, raw transactions, receipts, and block index; startup verifies genesis identity, manifest checksum/counts, and recomputed state/tx/receipt roots. Abrupt-exit, forced-kill, missing-entry, receipt-corruption, and injected-write-failure tests are fail closed. Format-1 snapshots are capped at 512 MiB and independently bind genesis, trusted app hash, state/content roots, checksum, transactions, and receipts. | Initial transactional crash-consistent version store and verified snapshot implementation; currently full-state-per-height archive-only storage | P0: incremental MVCC/deltas, deterministic migrations, pruned/full/archive guarantees, historical reads, compaction/load evidence, backup/restore, broader filesystem power-loss coverage, and rollback protection. |
| Networking | The production path has independent CometBFT homes, full-mesh persistent peers, flood-mempool gossip, real four-validator rounds, 3-of-4 progress, restart, block sync, and destructive-data state sync via two light-client RPC sources. The restored validator rejoins consensus and broadcasts the next transaction. | Initial CometBFT private-network block/state-sync path; discovery policy, evidence propagation, partitions, sentries, and load remain incomplete | Complete discovery/peer policy, evidence propagation, sentry topology, partition/Byzantine tests, ingress bounds, and load validation. |
...
| Validator lifecycle | Explicit genesis-fixed set; join/leave fail before admission, and signature-verified double-sign evidence consumes one chain/validator/height offence and clears stake without immediately changing that fixed set | Deliberately frozen harness lifecycle | P0: implement certified epoch transitions, voting-power updates, unbonding, evidence age/retention, and remote-signer monotonic last-sign state through CometBFT; do not re-enable ad hoc state-driven membership. |
...
| Security evidence | The full ABCI++/Comet/Pebble process tree passes unit/race suites, three repeated eight-process restart/block-sync/state-sync runs, three repeated snapshot suites, abrupt-exit/forced-kill tests, vet, tidy-diff, three production builds, fresh-process vectors, demo, and diff checks. Fixed `govulncheck` v1.6.0 reports zero reachable vulnerabilities, with two imported-package and 20 required-module advisories not reaching vulnerable symbols. CometBFT's module sums match signed sumdb; Windows directory verification remains non-green because the upstream zip contains a trailing-space filename. | Strong Windows development regression, fault, and signed dependency evidence; insufficient for hostile deployment | P0: broader fuzz/property/fault/cross-target evidence, malicious WASM/RPC/raw-tx/snapshot corpora, benchmarks/load/soak, clean Linux verification, threat model, and external review. |
...
- Preserve the implemented ChainLab ABCI++/CometBFT authoritative block, block-sync, state-sync, and snapshot lifecycle while adding validator updates, evidence slashing, partitions, and upgrade compatibility in multi-process networks.
- Evolve the implemented transactional version store from complete per-height state copies to incremental MVCC with migrations, retention profiles, historical reads, compaction, and backup/restore before public testnet.

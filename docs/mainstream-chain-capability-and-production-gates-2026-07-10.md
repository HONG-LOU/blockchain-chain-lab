...
| ABCI++ application lifecycle | CometBFT v0.39.3 is pinned with signed checksum evidence. The in-memory `chainlab-v1` app strictly binds genesis/consensus parameters, implements the full interface with bounded proposal/mempool work, publishes only on commit, and passes fresh-process/local-harness differential tests. Official socket-client coverage and an eight-process four-validator network now prove P2P rounds, transaction commitment, 3-of-4 progress, Comet restart, lagging-node block sync, fresh-app replay, common-height block/app-hash equality, and current state-root/account convergence. Snapshot restore is explicitly rejected. | Initial authoritative multi-process application/consensus lifecycle with replay-only application recovery; not durable or state-sync capable | P0: persist height/app hash/state atomically, implement verified snapshots/state sync, and prove validator updates, evidence slashing, partitions, Byzantine faults, load, and rolling upgrades. |
...
| Persistence | Snapshot v2 binds version, generation, checksum, explicit genesis time, state, canonical/known blocks, finality lock/votes/evidence, and a separate checksummed manifest; recovery is size-bounded, fail-closed, and replays roots from genesis before publishing state. Writes fsync a temporary file, rename it, then fsync the directory; a post-rename directory-fsync failure is commit-uncertain and enters a persistent sticky HALT rather than continuing from rolled-back memory. | Hardened whole-chain JSON development snapshot with fail-stop containment, still not a production database or proof of crash durability | P0: embedded transactional KV with atomic state/receipt/index/app-hash batches, WAL/reopen and kill/power-loss injection, deterministic migrations, independently verified snapshots, and explicit pruned/full/archive modes. Preserve fail-closed manifest/finality invariants, prove filesystem-specific durability, and define operator recovery from commit-uncertain HALT. |
| Networking | The local harness retains HTTP relay/import. The production path now has generated independent CometBFT homes, full-mesh persistent peers, flood-mempool gossip, real four-validator rounds, 3-of-4 progress, restart, and block-sync evidence. | Initial CometBFT private-network path; discovery, state sync, evidence propagation, partitions, sentries, and load remain incomplete | Complete discovery/peer policy, evidence propagation, state sync, sentry topology, partition/Byzantine tests, ingress bounds, and load validation. |
...
| Security evidence | The full ABCI++/Comet process tree passes unit/race suites, three repeated eight-process restart/sync runs, vet, tidy-diff, three production builds, fresh-process vectors, demo, and diff checks. Fixed `govulncheck` v1.6.0 first exposed three reachable node-graph advisories; `quic-go` v0.59.1 and gRPC-Go v1.79.3 close two, while `go-libp2p` v0.48.0 removes the unpatched `pion/dtls/v2` path in favor of v3. The final scan reports zero reachable vulnerabilities, with two imported-package and 20 required-module advisories not reaching vulnerable symbols. CometBFT's module sums match signed sumdb; Windows directory verification remains non-green because the upstream zip contains a trailing-space filename. | Strong Windows development regression and signed dependency evidence; insufficient for hostile deployment | P0: broader fuzz/property/fault/cross-target evidence, malicious WASM/RPC/raw-tx corpora, benchmarks/load/soak, maintained dependency scanning, clean Linux verification, threat model, and external review. |

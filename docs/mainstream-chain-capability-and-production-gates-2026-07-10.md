...
| ABCI++ application lifecycle | CometBFT v0.39.3 is pinned with signed checksum evidence. `chainlab-v1` strictly binds genesis/consensus parameters and the crash-consistent block/state-sync lifecycle. Explicit `chainlab-v2` additionally binds validator identity/evidence policy, persists one-time offence state through Pebble and snapshots, and returns epoch-bound power-zero updates with Comet's `H+2` timing. A real four-validator process test broadcasts valid duplicate-vote evidence and converges to three validators. | Initial authoritative crash-consistent multi-process application/consensus/block-sync/state-sync/evidence-removal lifecycle | P0: complete certified validator admission/re-entry and economics; prove partitions, additional Byzantine faults, load, and rolling upgrades; complete storage retention/migration and validator-signing gates. |

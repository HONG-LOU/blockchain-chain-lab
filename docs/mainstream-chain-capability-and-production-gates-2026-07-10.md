...
- Preserve the implemented ChainLab ABCI++/CometBFT authoritative block, block-sync, state-sync, snapshot, duplicate-vote slashing, epoch-removal, targeted missing-proposer, quorum-loss, and symmetric partition/heal lifecycle while adding certified admission/re-entry, dynamic message faults/asymmetric partitions, additional Byzantine evidence, and upgrade compatibility in multi-process networks.

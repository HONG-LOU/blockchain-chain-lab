...
| Networking | The production path has independent CometBFT homes, full-mesh persistent peers, flood-mempool gossip, real four-validator rounds, 3-of-4 progress, targeted missing round-0 proposer recovery at a later round, 2-of-4 quorum-loss recovery, symmetric 2+2 P2P partition isolation/healing, restart, block sync, destructive-data state sync via two light-client RPC sources, and duplicate-vote evidence broadcast/propagation followed by live four-to-three and simultaneous four-to-two validator transitions. | Initial CometBFT private-network block/state-sync/evidence path; validator outage, missing-proposer round advancement, simultaneous evidence-driven removal, and symmetric topology partition behavior are tested, while discovery policy, dynamic message faults/asymmetric partitions, sentries, and load remain incomplete | Complete discovery/peer policy, light-client-attack and packet-level delay/drop/reorder coverage, asymmetric partitions, delayed-but-connected proposers, non-removal power/admission changes, sentry topology, additional Byzantine tests, ingress bounds, and load validation. |

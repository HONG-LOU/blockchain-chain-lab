...
| Networking | The production path has independent CometBFT homes, full-mesh persistent peers, flood-mempool gossip, real four-validator rounds, 3-of-4 progress, targeted unavailable, connected-but-delayed, and malformed-proposal round-0 proposer recovery at later rounds, 2-of-4 quorum-loss recovery, symmetric 2+2 P2P partition isolation/healing, restart, block sync, destructive-data state sync via two light-client RPC sources, duplicate-vote evidence transitions, and Comet-verified same-height light-client equivocation followed by three-validator removal. | Initial CometBFT private-network block/state-sync/evidence path; validator outage, proposal-construction delay, invalid proposal rejection, simultaneous removal, light-client equivocation, and symmetric topology partition behavior are tested, while discovery policy, dynamic packet faults/asymmetric partitions, sentries, and load remain incomplete | Complete discovery/peer policy, forward/amnesia light-client cases, packet-level delay/drop/reorder coverage, asymmetric partitions, non-removal power/admission changes, sentry topology, additional Byzantine tests, ingress bounds, and load validation. |

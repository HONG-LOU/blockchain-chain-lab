...
3. Extend the multi-process network beyond implemented validator-outage, targeted missing-proposer, and symmetric partition recovery with dynamic message faults/asymmetric partitions, delayed-but-connected proposers, light-client attacks, simultaneous removals, Comet binary replacement, staged operator drills, and other Byzantine cases while preserving the implemented application rolling upgrade.

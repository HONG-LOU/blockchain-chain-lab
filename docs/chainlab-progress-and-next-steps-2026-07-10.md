...
1. **Authoritative CometBFT ABCI++ lifecycle**
   - The `chainlab-v1` lifecycle now runs across real socket and Comet node processes. Four-validator tests cover proposal/finalize/commit, full-mesh P2P, transaction gossip, 3-of-4 progress, 2-of-4 halt and recovery, Comet restart, block sync, durable app restart, fresh-app replay, destructive-data state sync, common-height block/app-hash equality, and current state-root convergence. Keep the local harness for deterministic differential testing only.
   - Preserve the implemented v2 evidence slashing, epoch-removal, quorum-loss recovery, and rolling application replacement paths while adding certified admission/re-entry, stake-derived power, unbonding/rewards, Comet binary/staged rolling drills, actual network partitions/message faults, delayed/missing proposer, light-client-attack, and broader Byzantine fault tests.
...
3. Extend the multi-process network beyond implemented validator-outage/quorum recovery with actual partitions/message faults, missing proposers, light-client attacks, simultaneous removals, Comet binary replacement, staged operator drills, and other Byzantine cases while preserving the implemented application rolling upgrade.

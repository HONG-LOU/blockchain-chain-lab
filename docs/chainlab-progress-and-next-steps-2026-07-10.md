...
Execution update on 2026-07-14: the Phase 0 contract, Phase 1 lifecycle, Phase 2 invitation/validator/observer flow, Phase 3 bounded retention/recovery/status, and Phase 4 software packaging/documentation/browser paths are implemented. RC2's Windows/Linux packages, five-container run, browser evidence, accelerated retention evidence, and prior software gates remain useful, but its formal 24-hour soak failed at height 10,020 on a transient Windows private-validator-state replacement conflict. The retry and outer-node failure-propagation fix passes direct reproduction and the integrated software baseline. A new RC and a fresh complete soak are required. The exact Windows reference-machine, separately operated physical-home WAN/power, Windows sleep/resume/interrupted-host, and 24-hour release-cadence soak gates remain open and prevent claiming the complete Desktop v0.1 milestone.

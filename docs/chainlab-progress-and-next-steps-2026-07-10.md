...
1. **Authoritative CometBFT ABCI++ lifecycle**
   ...
   - Preserve the implemented v2 duplicate-vote/light-client-equivocation slashing, epoch-removal, missing-proposer, quorum-loss/partition recovery, and rolling application replacement paths while adding certified admission/re-entry, stake-derived power, unbonding/rewards, Comet binary/staged rolling drills, dynamic message faults/asymmetric partitions, delayed-but-connected proposers, forward/amnesia light-client cases, and broader Byzantine fault tests.
...
3. Extend the multi-process network beyond implemented validator-outage, targeted missing-proposer, simultaneous evidence-driven removals, same-height light-client equivocation, and symmetric partition recovery with dynamic message faults/asymmetric partitions, delayed-but-connected proposers, forward/amnesia light-client cases, non-removal power/admission changes, Comet binary replacement, staged operator drills, and other Byzantine cases while preserving the implemented application rolling upgrade.

...
- This is a crash-consistent incremental application/network lifecycle, not a completed production network. Store V2 removes complete per-height disk copies, but commit-time flatten/diff still traverses current state. Mutation-aware delta generation, wider power-loss phases, operational restore drills, external rollback protection, partitions, Byzantine faults, rolling upgrades, remote signing, and Linux load/soak evidence remain open production gates. The exact storage contract is in `docs/chainlab-application-storage-v2.md`.
...
2. **Crash-consistent transactional storage**
   ...
   - Replace commit-time full-state flatten/diff with mutation-aware generation; add production-size compaction/retention measurements, an operational restore drill, additional filesystem power-loss phases, and external rollback protection.
...
2. Optimize Store V2 delta generation, then add production-size retention/compaction/load evidence, broader power-loss coverage, an operational restore drill, and external rollback protection.

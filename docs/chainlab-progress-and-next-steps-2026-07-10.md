...
5. **Resource and RPC governance**
   - Add gas-bounded/lazy proposal simulation, deterministic eviction/TTL, a production WAL-backed journal without full-snapshot write amplification, and orphaned-transaction reinsertion; preserve the implemented snapshot-backed txpool restart and incremental byte accounting across every mutation and rollback path.

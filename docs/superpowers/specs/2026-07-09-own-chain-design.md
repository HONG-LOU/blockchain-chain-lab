...
The canonical store is in-memory with snapshot cloning. The development node persists a checksummed snapshot v2 plus manifest and replays from genesis on restart; this is not the transactional versioned KV backend required for production.
...
- `--data-dir` uses a checksummed snapshot v2 and manifest with bounded fail-closed decoding and replay from persisted genesis. Writes fsync a temporary file, rename it, and fsync the directory; a post-rename directory-fsync failure is commit-uncertain and enters persistent sticky HALT. This contains ambiguity but is not a production database or completed crash-durability gate.

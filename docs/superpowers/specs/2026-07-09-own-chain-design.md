...
- RPC keeps an in-memory EVM-style filter registry for polling new block hashes, newly seen pending transaction hashes, and incremental event changes. Filters are node-local and restart-volatile, matching the development-chain scope; broader WebSocket subscriptions and a separate external indexer remain later milestones.
- `GET /rpc/ws` supports WebSocket JSON-RPC `eth_subscribe("newHeads")` plus `eth_unsubscribe`, broadcasting EVM-style block header payloads after local production or canonical peer import. Log and pending transaction WebSocket subscriptions remain later milestones.

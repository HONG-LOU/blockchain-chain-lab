...
Invalid transactions and current execution failures are rejected before block inclusion when possible. The present executor discards contract effects but also leaves nonce and fees unchanged on OOG, trap, or native-contract failure. This is not the target production behavior: admission-invalid transactions must be distinguished from included execution failures, and the latter must receive a deterministic failed receipt, consume nonce/gas, and pay fees while reverting business state.
...
- WASM upload gas scales with bytecode size and rejects modules above 512 KiB before decoding/compilation. Metering version `chainlab-wasm-v1` pins Wasmtime-Go v46.0.1, prices code bytes plus fixed declared memory/table resources before instantiation, uses engine fuel for guest execution, deducts host work from the same remaining budget, rejects start/WASI/unknown ABI and growable resources, bounds stack/host I/O/events/storage writes and growth, isolates reads, and rolls back failed direct or transaction calls. Runtime upgrades are consensus upgrades. Remaining mainnet gates include deterministic guest call depth, charged failures, hard JIT memory bounds, Linux/amd64 golden replay, reproducible native artifacts/SBOM, fuzz/load/dependency evidence, activation machinery, and external review.

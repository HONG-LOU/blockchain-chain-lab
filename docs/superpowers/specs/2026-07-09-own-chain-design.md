...
- RPC keeps an in-memory EVM-style filter registry for polling new block hashes, newly seen pending transaction hashes, and incremental event changes. Filters and WebSocket subscriptions are node-local and restart-volatile, matching the development-chain scope; pending transaction WebSocket subscriptions and a separate external indexer remain later milestones.
- `GET /rpc/ws` supports WebSocket JSON-RPC `eth_subscribe("newHeads")` and filtered `eth_subscribe("logs")` plus `eth_unsubscribe`, broadcasting EVM-style block header payloads and matching log notifications after local production or canonical peer import. Pending transaction WebSocket subscriptions remain a later milestone.

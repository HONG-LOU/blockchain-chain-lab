...
- RPC keeps an in-memory EVM-style filter registry for polling new block hashes, newly seen pending transaction hashes, and incremental event changes. Filters are node-local and restart-volatile, matching the development-chain scope; WebSocket subscriptions and a separate external indexer remain later milestones.
...
- `eth_getBlockByHash`, block transaction-count methods, and block+index transaction lookup methods project known ChainLab blocks into the same EVM-style block and transaction response shapes used by existing number/hash reads. Missing blocks or out-of-range transaction indexes return `null`.
...
- RPC and CLI expose mempool state through `GET /txpool`, `txpool_status`, `txpool_content`, and `query mempool`; `eth_getTransactionCount(..., "pending")` replays pending transactions on a cloned state so wallet-style commands can submit multiple uncommitted transactions with sequential nonces. `eth_getTransactionByHash` now falls back to txpool pending transactions with null block fields, `eth_newPendingTransactionFilter` lets clients poll for newly seen pending transaction hashes, and `eth_getTransactionReceipt` remains `null` until the transaction is included in a block.
...
- Blocks now carry `base_fee_per_gas`, `gas_limit`, and `gas_used`; transactions can use legacy `gas_price` or EIP-1559-style `max_fee_per_gas` / `max_priority_fee_per_gas`; receipts record effective gas price, burned base fee, and proposer priority fee. RPC exposes `eth_gasPrice`, `eth_maxPriorityFeePerGas`, `eth_feeHistory`, `chain_feeMarket`, and EVM block fee fields, while CLI exposes `query fees` and capped transfer flags. `eth_feeHistory` derives base fee history, gas-used ratios, next base fee, and optional gas-weighted priority-fee reward percentiles from canonical blocks and receipts. This models the fee market without claiming full Ethereum typed raw transaction compatibility.

...
- Transaction types for transfer, batched transfer/call user operations, EIP-7702-style `set_code` delegation, contract deployment, contract calls, staking, unstaking, proposal submission, proposal execution, and governance voting. Transactions can optionally carry a `signer` for single-owner contract-account or delegated-EOA authorization, or `authorizations` for multisig threshold accounts.

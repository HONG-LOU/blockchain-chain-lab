...
- RPC keeps an in-memory EVM-style filter registry for polling new block hashes and incremental event changes. Filters are node-local and restart-volatile, matching the development-chain scope; WebSocket subscriptions and a separate external indexer remain later milestones.

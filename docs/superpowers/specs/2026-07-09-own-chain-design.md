...
- `account.v1` accounts and delegated EOAs support ChainLab-native transfer session keys through `account.session_key`. The owner installs or revokes a session key, and the session key can then sign value transfers from the account within a stored value limit, optional recipient allowlist, and optional block-height expiry. The account still owns nonce, balance, and fee liability, and spent value is tracked in account storage. This is not ERC-4337 `UserOperation`, ERC-7579 modular account compatibility, contract-call session policy, social recovery, or a programmable policy engine.

...
- `validator.slash` accepts only two different canonical block hashes at the same chain/height with valid low-s signatures from the target validator. The local-harness evidence ID canonicalizes the sorted pair, while one persistent offence tombstone per chain/validator/height prevents alternate old vote pairs from slashing newly staked funds. The Comet application separately uses V2 evidence-driven removal; V5 timestamped offences compact only when block and duration retention both expire, while legacy tombstones remain permanent. Slashing clears current local-harness stake and deliberately does not mutate its fixed validator set. Unbonding, stake-derived voting power, rewards, admission, and re-entry remain production lifecycle work.

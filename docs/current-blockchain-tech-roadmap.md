...
- The explicit `chainlab-v2` path binds validator identities and evidence policy, applies one-time Comet-authenticated slashing before transaction execution, and schedules deterministically sorted validator updates on epoch boundaries using Comet's `H+2` activation rule. Genesis-bound `2/3+1` certificates can pre-authorize future admissions whose power is fixed by `floor(genesis_stake/1000)`. Optional `unbonding_epochs>=2` releases remaining stake only after that many complete epochs following evidence removal; zero and old snapshots remain permanently locked. Certificate tampering, cross-chain replay, minority approval, duplicates, stake/power mismatch, and schedule violations fail closed. Real four-validator networks prove both a single 4-to-3 removal and a simultaneous two-update 4-to-2 transition followed by continued transaction progress and four-full-node convergence. Runtime admission/re-entry and power changes, voluntary-leave unbonding/rewards, remote signing, and the broader fault network remain open.

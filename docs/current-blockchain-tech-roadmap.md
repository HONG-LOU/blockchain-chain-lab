...
- The `chainlab-v1` CometBFT v0.39.3 ABCI++ application strictly binds canonical genesis and consensus parameters, reuses the deterministic executor, performs bounded prepare and full process/finalize replay, and publishes only after a synchronized Pebble WAL batch atomically binds versioned state, height/app hash, transactions, receipts, and block index. Official socket and four-validator process tests cover P2P rounds, raw transaction commitment, 3-of-4 progress, targeted round-0 proposer outage with later-round commit, 2-of-4 quorum-loss halt/recovery, symmetric 2+2 P2P partition with side-local mempools and bridge healing, Comet restart, block sync, durable app restart, fresh-app replay, destructive-data state sync through two trusted RPC sources, and cross-node convergence.
- The explicit `chainlab-v2` path binds validator identities and evidence policy, applies one-time Comet-authenticated slashing before transaction execution, and schedules deterministically sorted validator updates on epoch boundaries using Comet's `H+2` activation rule. Genesis-bound `2/3+1` certificates can pre-authorize future admissions whose power is fixed by `floor(genesis_stake/1000)`. Optional `runtime_admissions` accepts candidate-paid join transactions authorized by active voting power strictly above two thirds and bound to the preceding validator root/height; optional `unbonding_epochs>=2` releases remaining stake only after that many complete epochs following evidence removal. Both default off for compatibility. Certificate tampering, cross-chain replay, minority approval, duplicates, stale roots, stake/power mismatch, schedule violations, and under-metered certificates fail closed. Real four-validator networks prove removal and runtime-admission transitions. Re-entry, runtime power changes, voluntary-leave unbonding/rewards, remote signing, and the broader fault network remain open.
- Application Store V2 keeps a flat live state plus mutation-journal-derived per-height deltas and periodic checkpoints in one synchronized Pebble batch with results, commitment, current pointer, history boundary, and block index. Store clones preserve semantic dirty keys until durable commit; checkpoint heights compare the journal against a complete diff and fail closed on mismatch. V4 ordinary commits also update a copy-on-write sparse accumulator from the journal and publish it only after durable persistence; checkpoints rebuild and cross-check the complete sparse root. Deterministic V1 migration, exact archive/full/pruned identities, historical ABCI reads, compaction, consistent checkpoint backups, state-sync history rebasing, corruption rejection, and process-kill atomicity evidence are implemented. Linux capacity/load/soak, wider filesystem faults, external rollback protection, and an operational restore drill remain open.
- A canonical V2 genesis can schedule sequential V3, V4, and V5 activation heights. V3 introduces exact-total roots; V4 moves state to mutation-aware sparse proofs; V5 records evidence time on new validator offences and compacts them only after block-age and duration-age both exceed the genesis policy. Legacy V2-V4 tombstones remain permanent. Current/historical queries survive Pebble restart and snapshot/state sync; fixed fresh-process vectors, incompatible-version rejection, a real four-validator V3/V4/V5 upgrade/proof test, and pre-activation one-at-a-time replacement from V4-capped to V5 applications with 3-of-4 transaction progress are implemented. Scheduling is genesis-only; runtime governance authority, production proof serving, an external verifier implementation, signed releases, Comet binary upgrades, and staged rolling operations remain open.
...
- The local harness now persists a checksum- and generation-bound snapshot v3 plus a data-directory manifest, replays canonical and known branches from an explicitly timestamped genesis, validates every replayed state root, revalidates snapshot-bound pending/queued transactions, migrates v2 deterministically, and refuses silent genesis recreation or manifest rollback when initialized data is incomplete. Public genesis and validator secrets are separate artifacts. This is fail-closed development persistence, not the transactional production database.
...
- The local harness and `chainlab-v1` set remain fixed by genesis. `validator.join` and `validator.leave` stay rejected. `chainlab-v2` supports Comet-authenticated evidence-driven epoch removal with persistent one-time validator-height tombstones, but no arbitrary membership or complete staking lifecycle.
...
- HTTP and JSON-RPC ingress, all-request/POST/RPC concurrency, batch recording, txpool queries, fee-history, log/filter definitions and TTL, pending-filter retained hashes, WebSocket lifecycle/subscription TTL/log work, transaction, block, validator, chain-ID, known-fork, snapshot, WASM, and per-account storage have explicit limits. Explorer pending and queued entries link to bounded pool-aware detail pages instead of dead committed-only routes. Published state views are immutable, and long `eth_call` reads no longer hold the node lock or clone the whole state.
...
- Preserve V3 exact-total proofs and V4 sparse membership/non-membership proofs across all flat-state kinds; add light-client integration, an external independent implementation, isolated proof serving, and load evidence.
...
- Extend the implemented normal-round, 3-of-4 progress, targeted missing/connected-delayed/invalid proposer round advancement, 2-of-4 halt/recovery, symmetric partition/heal, reconnect, restart, app-replay, block-sync, state-sync, runtime admission, single/simultaneous duplicate-vote, same-height light-client-equivocation and cross-height forward-lunatic removals, and rolling-application process tests with dynamic packet delay/drop/reorder and asymmetric partitions, amnesia evidence, external light-client detection/sourcing, broader Byzantine proposal behavior, re-entry/non-removal power changes, load, Comet binary upgrades, and staged operator drills.
...
- Preserve snapshot-v3 checksum, v2 migration, manifest rollback protection, replay, finality-lock, evidence, txpool restart, and fail-closed schema invariants while the local harness remains a differential tool.
...
- Preserve the implemented flat-live-state, incremental per-height delta, periodic checkpoint, and atomic retention layout; measure and tune compaction under production-size state/history.
- Extend the implemented abrupt-exit/forced-kill recovery across additional filesystem and commit phases.
- Preserve deterministic V1-to-V2 migration, consistent backup/open tests, and independently verified snapshot/state-sync import; add an operational restore drill and external rollback protection.
- Preserve the implemented pruned, full, and archive guarantees, profile identity checks, mutation-aware delta generation, V4 incremental authenticated state, and checkpoint journal/root cross-checks; add production-scale Linux compaction/load/soak evidence before claiming throughput.
...
- Add deterministic txpool eviction/TTL, a production WAL-backed txpool journal without full-snapshot write amplification, principal-aware rate limits, peer quotas, pagination, and bounded future internal queues. Preserve the implemented snapshot-backed txpool restart, bounded deterministic orphaned-transaction reinsertion after reorg, incremental byte accounting, authorization-principal-safe replacement, all-request/POST/filter/subscription limits, active filter and WebSocket subscription expiry, WebSocket heartbeat/write timeout/work budget, and HTTP timeouts.
...
- Preserve the implemented genesis-committed V2-to-V3-to-V4-to-V5 activation, Comet app-version updates, root/lifecycle migrations, incompatible-node rejection, V5 offence retention, and automated application rolling replacement; add runtime-authorized scheduling, signed binary manifests, rollback limits, Comet binary upgrades, and staged operator runbook drills.

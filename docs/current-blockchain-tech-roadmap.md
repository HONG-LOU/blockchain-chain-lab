...
- Extend the implemented normal-round, 3-of-4 progress, targeted missing/connected-delayed/invalid proposer round advancement, 2-of-4 halt/recovery, symmetric partition/heal, reconnect, restart, app-replay, block-sync, state-sync, single/simultaneous duplicate-vote, same-height light-client-equivocation and cross-height forward-lunatic removals, and rolling-application process tests with dynamic packet delay/drop/reorder and asymmetric partitions, amnesia evidence, external light-client detection/sourcing, broader Byzantine proposal behavior, non-removal power/admission changes, load, Comet binary upgrades, and staged operator drills.
...
- Add deterministic eviction/TTL, a production WAL-backed txpool journal without full-snapshot write amplification, principal-aware rate limits, WebSocket subscription TTL, peer quotas, pagination, and bounded future internal queues. Preserve the implemented snapshot-backed txpool restart, bounded deterministic orphaned-transaction reinsertion after reorg, incremental byte accounting, authorization-principal-safe replacement, all-request/POST/filter/subscription limits, active filter expiry, WebSocket heartbeat/write timeout/work budget, and HTTP timeouts.

...
- A canonical V2 genesis can schedule sequential V3 and V4 activation heights. V3 introduces domain-separated exact-total transaction/receipt/flat-state roots. V4 retains transaction/receipt roots and moves state to mutation-aware sparse membership/non-membership proofs across all flat kinds. Current/historical queries survive Pebble restart and snapshot/state sync; the standalone verifier auto-detects both envelope formats. Fixed fresh-process vectors, incompatible-version rejection, and a real four-validator V3/V4 upgrade/proof test are implemented. Scheduling is genesis-only; runtime governance authority, production proof serving, an external verifier implementation, and rolling-upgrade operations remain open.
...
- Preserve the implemented genesis-committed V2-to-V3-to-V4 activation, Comet app-version updates, root migrations, and incompatible-node rejection; add runtime-authorized scheduling, binary manifests, rollback limits, and operator rolling-upgrade drills.

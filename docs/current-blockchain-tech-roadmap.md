...
- Application Store V2 keeps a flat live state plus mutation-journal-derived per-height deltas and periodic checkpoints in one synchronized Pebble batch with results, commitment, current pointer, history boundary, and block index. Store clones preserve semantic dirty keys until durable commit; checkpoint heights compare the journal against a complete diff and fail closed on mismatch. It provides deterministic V1 migration, exact archive/full/pruned identities, historical ABCI reads, compaction, consistent checkpoint backups, state-sync history rebasing, corruption rejection, and process-kill atomicity evidence. Flat integrity/proof roots and checkpoints still traverse current state, while Linux capacity/load/soak, wider filesystem faults, external rollback protection, and an operational restore drill remain open.
- A canonical V2 genesis can schedule one V3 activation height. `FinalizeBlock(A-1)` updates Comet application version, height `A` switches to domain-separated transaction/receipt/flat-state Merkle roots, and current/historical proof queries survive Pebble restart and snapshot/state sync. A standalone trusted-root verifier, fixed fresh-process vectors, incompatible-version rejection, and a real four-validator upgrade/proof test are implemented. Scheduling is genesis-only; runtime governance/multi-upgrade policy, sparse/non-inclusion proofs, production proof serving, an external verifier implementation, and rolling-upgrade operations remain open.
...
- Preserve the implemented V3 transaction/receipt/account-state inclusion proofs and standalone trusted-root verifier; add non-inclusion/sparse proofs, more state routes, light-client integration, an external independent implementation, and load evidence.
...
- Preserve the implemented pruned, full, and archive guarantees, profile identity checks, mutation-aware delta generation, and checkpoint journal cross-checks; add incremental authenticated flat/proof roots before claiming production throughput.
...
- Preserve the implemented genesis-committed V2-to-V3 activation, Comet app-version update, root migration, and incompatible-node rejection; add runtime-authorized multi-upgrade scheduling, binary manifests, rollback limits, and operator rolling-upgrade drills.

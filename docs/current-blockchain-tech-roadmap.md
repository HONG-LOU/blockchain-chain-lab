...
- The explicit `chainlab-v2` path binds validator identities and evidence policy, applies one-time Comet-authenticated slashing before transaction execution, and schedules deterministically sorted power-zero updates on epoch boundaries using Comet's `H+2` activation rule. Real four-validator networks prove both a single 4-to-3 removal and a simultaneous two-update 4-to-2 transition followed by continued transaction progress and four-full-node convergence. This is evidence-driven removal only; certified admission/re-entry, stake-derived power, unbonding/rewards, remote signing, and the broader fault network remain open.

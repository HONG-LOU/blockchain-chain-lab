...
- The explicit `chainlab-v2` path binds validator identities and evidence policy, applies one-time Comet-authenticated slashing before transaction execution, and schedules power-zero updates on epoch boundaries using Comet's `H+2` activation rule. Direct persistence/snapshot tests and a real four-validator duplicate-vote broadcast prove convergence from four validators to three. This is evidence-driven removal only; certified admission/re-entry, stake-derived power, unbonding/rewards, remote signing, and the broader fault network remain open.
...
- Extend the implemented normal-round, 3-of-4 progress, targeted missing-proposer round advancement, 2-of-4 halt/recovery, symmetric partition/heal, reconnect, restart, app-replay, block-sync, state-sync, real duplicate-vote, and rolling-application process tests with delayed-but-connected proposers, dynamic delay/drop/reorder and asymmetric partitions, simultaneous changes, light-client attacks, load, Comet binary upgrades, and staged operator drills.

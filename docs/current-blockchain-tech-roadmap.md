...
- A canonical V2 genesis can schedule sequential V3, V4, and V5 activation heights. V3 introduces exact-total roots; V4 moves state to mutation-aware sparse proofs; V5 records evidence time on new validator offences and compacts them only after block-age and duration-age both exceed the genesis policy. Legacy V2-V4 tombstones remain permanent. Current/historical queries survive Pebble restart and snapshot/state sync; fixed fresh-process vectors, incompatible-version rejection, and a real four-validator V3/V4/V5 upgrade/proof test are implemented. Scheduling is genesis-only; runtime governance authority, production proof serving, an external verifier implementation, and rolling-upgrade operations remain open.
...
- Extend the implemented normal-round, reconnect, restart, app-replay, block-sync, state-sync, and real duplicate-vote process tests with delayed/missing proposers, partitions, simultaneous changes, light-client attacks, load, and rolling upgrades.
...
- Preserve the implemented genesis-committed V2-to-V3-to-V4-to-V5 activation, Comet app-version updates, root/lifecycle migrations, incompatible-node rejection, and V5 offence retention; add runtime-authorized scheduling, binary manifests, rollback limits, and operator rolling-upgrade drills.

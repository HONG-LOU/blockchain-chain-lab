...
- Add deterministic eviction/TTL, a production WAL-backed txpool journal without full-snapshot write amplification, orphaned-transaction reinsertion after reorg, principal-aware rate limits, WebSocket subscription TTL, peer quotas, pagination, and bounded future internal queues. Preserve the implemented snapshot-backed txpool restart, incremental byte accounting, authorization-principal-safe replacement, all-request/POST/filter/subscription limits, active filter expiry, WebSocket heartbeat/write timeout/work budget, and HTTP timeouts.

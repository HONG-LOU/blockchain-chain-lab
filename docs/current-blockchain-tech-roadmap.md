...
- Application Store V2 keeps a flat live state plus per-height deltas and periodic checkpoints in one synchronized Pebble batch with results, commitment, current pointer, history boundary, and block index. It provides deterministic V1 migration, exact archive/full/pruned identities, historical ABCI reads, compaction, consistent checkpoint backups, state-sync history rebasing, corruption rejection, and process-kill atomicity evidence. Commit-time snapshot flattening/diff is still O(current state), and Linux capacity/load/soak, wider filesystem faults, external rollback protection, and an operational restore drill remain open.
...
- Preserve the implemented pruned, full, and archive guarantees and profile identity checks; optimize full-state flatten/diff into mutation-aware incremental generation before claiming production throughput.

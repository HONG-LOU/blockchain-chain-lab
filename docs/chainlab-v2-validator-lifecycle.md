...
`ValidatorPolicy.runtime_admissions` is a separate omitempty activation and defaults to false. When enabled, a candidate may submit `validator.join` with a canonical `chainlab-runtime-validator-admission-v1` certificate signed by active validators holding strictly more than two thirds of current voting power. The certificate binds the chain ID, immediately preceding committed height, current validator root, complete candidate identity, stake-derived power, and an epoch-aligned activation at least two heights ahead. Every join in one block is authorized against that same preceding committed root, rather than a root changed by an earlier join in the block. The candidate signs and pays for its own transaction; intrinsic gas includes the canonical certificate bytes and signature count. Stale roots/heights, minority or duplicate signers, inactive signers, cross-chain replay, malformed/non-canonical JSON, insufficient stake, duplicate identity, schedule mismatch, and underfunded gas fail without changing the lifecycle. FinalizeBlock derives Comet updates from the post-transaction lifecycle, so a successful join emits its positive-power update exactly two heights before activation. Disabled chains retain the earlier pre-execution join rejection.

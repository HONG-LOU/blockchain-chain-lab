...
The lifecycle state schema can now retain additional non-genesis identities with a positive future activation height while requiring every genesis identity to remain present. Identity count, account, compressed public key, consensus address, voting power, and uniqueness constraints remain enforced, and snapshot restore preserves the same validator root. No V1-V5 transaction or ABCI path can create or activate such an identity, so this storage foundation does not enable validator admission by itself.

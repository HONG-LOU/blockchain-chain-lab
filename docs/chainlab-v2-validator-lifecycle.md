...
`chainlab-v2` adds an explicitly versioned CometBFT evidence-to-slashing path and epoch-bound validator removal. Genesis-certified admission can derive initial power from stake, and a validator policy may explicitly activate delayed unbonding after evidence removal and certified runtime `validator.join`. Voluntary `validator.leave`, re-entry, runtime power changes, rewards, and governance-controlled set changes remain fail closed until separately activated transition protocols and economics rules exist.
...
A fifth real-process test halts a four-validator network at two online validators, constructs a candidate-paid runtime-admission transaction authorized by all four active validator keys against the last committed validator root, and proves the transaction cannot commit without quorum. Restoring a third validator commits the admission and emits the positive-power update two heights before the candidate's epoch-aligned activation. Restoring the fourth validator leaves more than two thirds of the expanded voting power online, after which all four applications and Comet RPC validator sets expose the candidate and commit a subsequent transaction. The candidate key is generated only for the isolated test and never represents an operational validator secret.
...
`ValidatorPolicy.runtime_admissions` is a separate omitempty activation and defaults to false. When enabled, a candidate may submit `validator.join` with a canonical `chainlab-runtime-validator-admission-v1` certificate signed by active validators holding strictly more than two thirds of current voting power. The certificate binds the chain ID, an earlier committed height, the unchanged current validator root, complete candidate identity, stake-derived power, and an epoch-aligned activation at least two heights after inclusion. Legacy chains and omitted `runtime_admission_window=0` retain the exact immediately preceding-height rule and bytes. New chains may explicitly set a positive window no larger than `epoch_length`; inclusion must then occur after authorization and within that window. This versioned policy lets the transaction cross real P2P/mempool propagation without silently changing old-chain execution or making old authority replayable. Every join in one block is authorized against the same pre-block committed root, rather than a root changed by an earlier join in the block. The candidate signs and pays for its own transaction; intrinsic gas includes the canonical certificate bytes and signature count. Future or out-of-window authorization heights, changed/stale roots, minority or duplicate signers, inactive signers, cross-chain replay, malformed/non-canonical JSON, insufficient stake, duplicate identity, schedule mismatch, and underfunded gas fail without changing the lifecycle. FinalizeBlock derives Comet updates from the post-transaction lifecycle, so a successful join emits its positive-power update exactly two heights before activation. Disabled chains retain the earlier pre-execution join rejection.

The lifecycle state schema retains certified non-genesis identities while requiring every genesis identity to remain present at activation height 1. Identity count, account, compressed public key, consensus address, voting power, uniqueness, and epoch-aligned admission/removal constraints remain enforced during proposal execution and persistent restore, and snapshot restore preserves the same validator root. The ABCI lifecycle emits each certified identity's deterministic positive-power update two heights before activation, alongside any zero-power removals. Proposal and vote-extension identity checks then resolve the active lifecycle instead of the frozen genesis map, so an activated validator is accepted only inside its recorded interval. V1 and runtime-admissions-disabled V2-V5 reject transaction-created identities; enabled V2-V5 accept only the certified flow above.

Production lifecycle completion still requires re-entry, runtime power changes, voluntary leave/unbonding, epoch transition history, reward accounting, evidence economics, remote signer/HSM protection, key rotation, quorum and emergency rules, governance/upgrade authority, broader admission partitions and rolling-upgrade tests, and adversarial economic simulation. V5 closes forward tombstone compaction; legacy tombstones deliberately remain permanent because their evidence time was never committed.

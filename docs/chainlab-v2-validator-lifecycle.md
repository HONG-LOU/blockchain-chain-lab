...
The lifecycle state schema can now retain additional non-genesis identities with a positive future activation height while requiring every genesis identity to remain present. Identity count, account, compressed public key, consensus address, voting power, and uniqueness constraints remain enforced, and snapshot restore preserves the same validator root. Once a separately certified path writes such an identity, the ABCI lifecycle emits its deterministic positive-power update two heights before activation, alongside any zero-power removals. Proposal and vote-extension identity checks then resolve the active lifecycle instead of the frozen genesis map, so an activated validator is accepted only inside its recorded interval. No V1-V5 transaction can create or authorize such an identity, so this activation plumbing does not enable validator admission by itself.

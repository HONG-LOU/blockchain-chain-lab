...
`ValidatorPolicy.runtime_admissions` is a separate omitempty activation and defaults to false. When enabled, a candidate may submit `validator.join` with a canonical `chainlab-runtime-validator-admission-v1` certificate signed by active validators holding strictly more than two thirds of current voting power. The certificate binds the chain ID, immediately preceding committed height, current validator root, complete candidate identity, stake-derived power, and an epoch-aligned activation at least two heights ahead. The candidate also signs and pays for its own transaction. Stale roots/heights, minority or duplicate signers, inactive signers, cross-chain replay, malformed/non-canonical JSON, insufficient stake, duplicate identity, and schedule mismatch fail without changing the lifecycle. FinalizeBlock derives Comet updates from the post-transaction lifecycle, so a successful join emits its positive-power update exactly two heights before activation. Disabled chains retain the earlier pre-execution join rejection.
...
Production lifecycle completion still requires re-entry, runtime power changes, voluntary leave/unbonding, epoch transition history, reward accounting, evidence economics, remote signer/HSM protection, key rotation, quorum and emergency rules, governance/upgrade authority, real multi-process runtime-admission evidence, partitions and rolling-upgrade tests, and adversarial economic simulation. V5 closes forward tombstone compaction; legacy tombstones deliberately remain permanent because their evidence time was never committed.
